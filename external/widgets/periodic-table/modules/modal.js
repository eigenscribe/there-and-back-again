  ...
  overlay.render = (el, gradients) => {
    ...
    box.innerHTML = `
        ...
        <div style="text-align: center; margin-bottom: 0.05rem;">
          ...
          <h4 style="margin-bottom: 0.02rem;">${el.name}</h4>
          <div style="font-size: 0.7rem; color: #fff; opacity: 0.8;">Atomic Number: ${el.number}</div>
        </div>
        <div class="modal-grid" style="margin-top: 0.05rem;">
          ...
          ${el.description ? `<div class="modal-item" style="grid-column:1/3; font-size: 0.75rem; line-height: 1.2;">${el.description}</div>` : ""}
        </div>
    `;
